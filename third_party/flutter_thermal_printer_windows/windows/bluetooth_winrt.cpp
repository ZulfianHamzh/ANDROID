...
static bool BluetoothSendImpl(const std::string& device_id, const uint8_t* data, size_t size) {
  ...
  try {
    ...
    // The socket is dead (e.g. aborted by the printer/OS) — drop it so a
    // fresh connection can be established on the next attempt.
    BluetoothDisconnectImpl(device_id);
    return false;
  } catch (const std::exception& e) {
...
// ── Bluetooth radio state (Windows.Devices.Radios) ────────────────

static winrt_win::Devices::Radios::Radio FindBluetoothRadio() {
  auto radios = winrt_win::Devices::Radios::Radio::GetRadiosAsync().get();
  for (auto radio : radios) {
    if (radio.Kind() == winrt_win::Devices::Radios::RadioKind::Bluetooth) {
      return radio;
    }
  }
  return nullptr;
...
bool BluetoothIsRadioOn() {
  bool on = false;
  RunOnMta([&]() {
    try {
      auto radio = FindBluetoothRadio();
      on = radio &&
           radio.State() == winrt_win::Devices::Radios::RadioState::On;
    } catch (const std::exception& e) {
      BT_LOG("BluetoothIsRadioOn ERROR: " << e.what());
    } catch (...) {
      BT_LOG("BluetoothIsRadioOn ERROR: unknown");
    }
  });
  return on;
}

bool BluetoothEnableRadio() {
  bool ok = false;
  RunOnMta([&]() {
    try {
      auto radio = FindBluetoothRadio();
      if (!radio) {
        BT_LOG("BluetoothEnableRadio: no Bluetooth radio found");
        return;
      }
      if (radio.State() == winrt_win::Devices::Radios::RadioState::On) {
        ok = true;
        return;
      }
      radio.SetStateAsync(winrt_win::Devices::Radios::RadioState::On).get();
      ok = radio.State() == winrt_win::Devices::Radios::RadioState::On;
      BT_LOG(ok ? "BluetoothEnableRadio: enabled" : "BluetoothEnableRadio: still off");
    } catch (const std::exception& e) {
      BT_LOG("BluetoothEnableRadio ERROR: " << e.what());
    } catch (...) {
      BT_LOG("BluetoothEnableRadio ERROR: unknown");
    }
  });
  return ok;
}
