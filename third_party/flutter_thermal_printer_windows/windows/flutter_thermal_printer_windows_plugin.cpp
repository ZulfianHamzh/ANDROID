...
void FlutterThermalPrinterWindowsPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("getPlatformVersion") == 0) {
    ...
    result->Success(flutter::EncodableValue(out));
  } else if (method_call.method_name().compare("isBluetoothEnabled") == 0) {
    result->Success(flutter::EncodableValue(BluetoothIsRadioOn()));
  } else if (method_call.method_name().compare("enableBluetooth") == 0) {
    result->Success(flutter::EncodableValue(BluetoothEnableRadio()));
  } else {
